...
export class GeneralService {
  ...
  async snapshot(): Promise<{ llm: ReturnType<LlmMetrics['snapshot']>; jobs: Awaited<ReturnType<Scheduler['counts']>>; inFlight: number; pendingDoctrines: number; classes: { voice: string | null; routine: string | null; narrative: string | null }; memory: { stores: string[]; mirrorFailures: number } }> {
    return { llm: this.metrics.snapshot(), jobs: await this.scheduler.counts(), inFlight: this.scheduler.inFlight, pendingDoctrines: this.pending.size, classes: { voice: this.stack.voice?.name ?? null, routine: this.stack.forClass('routine')?.name ?? null, narrative: this.stack.narrative?.name ?? null }, memory: { stores: this.mirror ? ['postgres', 'instance'] : ['postgres'], mirrorFailures: this.mirror?.mirrorFailures ?? 0 } };
  }
