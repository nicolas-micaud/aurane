...
export function renderGazettePage(issue: GazetteIssue | null, lang: 'fr' | 'en', summary: { colonies: { id: string; name: string; faction: string; score: number }[] }): string {
  if (!issue) {
  ...
  const board = `<h2>${lang === 'fr' ? 'Classement' : 'Standings'}</h2><table>${summary.colonies.slice(0, 10).map((c, i) => `<tr><td>${i + 1}</td><td><a href="/c/${esc(c.id)}" style="color:#d7dfee">${esc(c.name)}</a></td><td class="muted">${esc(c.faction)}</td><td>${c.score}</td></tr>`).join('')}</table>`;
...
export function renderColonyPage(c: { id: string; name: string; faction: string; persona: string; alliance: string | null; score: number; connected: number; npc: boolean; beacons: string[] }): string {
  const body = `<h1>${esc(c.name)}</h1><p class="lead">${esc(c.faction)}${c.alliance ? ` · ${esc(c.alliance)}` : ''}${c.npc ? ' · PNJ' : ''}</p><table><tr><th>Score</th><td>${c.score}</td></tr><tr><th>Systèmes</th><td>${c.connected}</td></tr><tr><th>Général</th><td>${esc(c.persona)}</td></tr>${c.beacons.length ? `<tr><th>Phares</th><td>${esc(c.beacons.join(', '))}</td></tr>` : ''}</table>`;
