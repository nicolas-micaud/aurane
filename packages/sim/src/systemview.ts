// The System view: one plateau, streamed at 2 Hz to whoever is looking at it, and the
// battle reports built from the salvo log. Fog of war: the sector must be visible; stocks,
// queues, docked fleets and routes are for the owner and allies only.
...
import type { BattleLog, Colony, PlateauPos, World } from './state.js';
import { combatSize, fleetSize } from './state.js';
import { capacityOf, orbitSlots } from './structures.js';
import { colonyNetwork, ownedSystems, visibleSectors } from './world.js';
...
export interface SystemDetailView {
  ...
  structures: { id: string; kind: Building; orbit: Orbit; angle: number; hp: number; maxHp: number; armed: boolean; range: number | null }[];
...
const orderName = (o: World['fleets'][string]['order']): string => o.kind;
...
export function systemViewFor(w: World, colony: Colony, systemId: string): SystemDetailView | null {
  ...
  const base: SystemDetailView = {
    ...
    time: w.time, owner: st.owner, ownerName: st.owner ? w.colonies[st.owner]?.name ?? null : null, mine, allied,
    connected: colonyNetwork(w, colony).has(systemId), visible, plateauRadius: B.PLATEAU_RADIUS,
    station: null, orbitSlots: [slots[1], slots[2], slots[3]], structures: [], stock: null, capacity: null, population: null,
  ...
  base.station = st.owner ? { pos: STATION_POS, hp: st.stationHp, maxHp: B.STATION_HP } : null;
  base.structures = st.structures.map((s) => {
    const t = B.TURRET_STATS[s.kind];
    return { id: s.id, kind: s.kind, orbit: s.orbit, angle: s.angle, hp: s.hp, maxHp: B.STRUCTURE_HP[s.kind], armed: !!t, range: t ? t.range : s.kind === 'bastion' ? B.BASTION_RANGE : null };
  });
  ...
  if (st.owner) {
    const owner = w.colonies[st.owner];
    const watch = owner ? ((Math.floor(w.time / 3600) % 24) - owner.watchStartHour + 24) % 24 < B.WATCH_HOURS : false;
    const bastion = st.structures.some((s) => s.kind === 'bastion' && s.hp > 0);
    base.shield = bastion ? { active: true, fraction: watch ? B.BASTION_SHIELD_WATCH : B.BASTION_SHIELD } : null;
  }
  if (detail) {
  ...
  for (const f of Object.values(w.fleets)) {
    ...
    if (f.at === systemId) {
      if (f.pos === null && !friend) continue; // docked inside the station: out of sight
      base.fleets.push({
        ...
        focus: friend ? f.focus : null, cargo: friend ? f.cargo : null, docked: f.pos === null,
      });
  ...
  const live = Object.values(w.battles).find((b) => b.system === systemId && b.endedAt === null);
  if (live) base.battle = { id: live.id, startedAt: live.startedAt, sides: live.sides, kills: killsBySide(live) };
  return base;
}
...
export interface BattleReport {
  id: string; system: string; systemName: string; startedAt: number; endedAt: number | null; seconds: number;
...
export function battleReport(w: World, colony: Colony, battleId: string): BattleReport | null {
  ...
  if (b.endedAt !== null) {
    if (!defenderId || st.owner !== defenderId) outcome = 'lost';
      ...
      const attackersLeft = Object.values(w.fleets).some((f) => f.at === b.system && f.pos !== null && f.owner !== defenderId && combatSize(f.units) > 0);
      outcome = attackersLeft || st.blockade ? 'lost' : b.events.some((e) => e.kind === 'kill' || e.kind === 'destroyed' || e.kind === 'station.down') ? 'held' : 'skirmish';
    }
  }
  return { id: b.id, system: b.system, systemName: w.galaxy.systems[b.system]?.name ?? b.system, startedAt: b.startedAt, endedAt: b.endedAt, seconds, sides, outcome, timeline: b.events.slice(-400) };
}

/** Recent engagements this colony may read, newest first. */
export function battleList(w: World, colony: Colony, limit = 30): { id: string; system: string; systemName: string; startedAt: number; endedAt: number | null; sides: string[]; kills: number }[] {
  ...
  return Object.values(w.battles)
    ...
    .map((b) => ({ id: b.id, system: b.system, systemName: w.galaxy.systems[b.system]?.name ?? b.system, startedAt: b.startedAt, endedAt: b.endedAt, sides: b.sides, kills: b.events.filter((e) => e.kind === 'kill').reduce((s, e) => s + (e.amount ?? 0), 0) }));
}
