...
export function fleetPower(f: Fleet, vs: Fleet): number {
  ...
  for (const t of TYPES) {
    const counteredShare = vs[COUNTERS[t]] / total;
...
export function resolveBattle(attacker: Fleet, defender: Fleet, defenseMult: number, seed: number): BattleResult {
  ...
  if (fleetSize(defender) === 0) {
    return { attackerWins: true, attackerLosses: { corvette: 0, frigate: 0, cruiser: 0 }, defenderLosses: { ...defender }, attackerPower, defenderPower };
  }
...
export function subtractFleet(f: Fleet, losses: Fleet): Fleet {
  return { corvette: f.corvette - losses.corvette, frigate: f.frigate - losses.frigate, cruiser: f.cruiser - losses.cruiser };
}

export function addFleet(a: Fleet, b: Fleet): Fleet {
  return { corvette: a.corvette + b.corvette, frigate: a.frigate + b.frigate, cruiser: a.cruiser + b.cruiser };
}
