...
export const DEFAULT_LIMITS: QuotaLimits = { talkPerHour: 20, talkPerDay: 60, doctrinePerDay: 12, briefingPerDay: 8 };

/** LLM_QUOTA_TALK_HOUR / _TALK_DAY / _DOCTRINE_DAY / _BRIEFING_DAY. */
export function limitsFromEnv(env: NodeJS.ProcessEnv = process.env): QuotaLimits {
  ...
  return { talkPerHour: n(env.LLM_QUOTA_TALK_HOUR, DEFAULT_LIMITS.talkPerHour), talkPerDay: n(env.LLM_QUOTA_TALK_DAY, DEFAULT_LIMITS.talkPerDay), doctrinePerDay: n(env.LLM_QUOTA_DOCTRINE_DAY, DEFAULT_LIMITS.doctrinePerDay), briefingPerDay: n(env.LLM_QUOTA_BRIEFING_DAY, DEFAULT_LIMITS.briefingPerDay) };
}

export type QuotaKind = 'talk' | 'doctrine' | 'briefing';

interface Bucket { hour: number; day: number; talkHour: number; talkDay: number; doctrineDay: number; briefingDay: number }
...
export class PlayerQuota {
  ...
  private bucket(colony: string): Bucket {
    ...
    if (!b || b.day !== day) { b = { hour, day, talkHour: 0, talkDay: 0, doctrineDay: 0, briefingDay: 0 }; this.buckets.set(colony, b); }
  ...
  remaining(colony: string): { talkHour: number; talkDay: number; doctrineDay: number; briefingDay: number } {
    const b = this.bucket(colony);
    return { talkHour: this.limits.talkPerHour - b.talkHour, talkDay: this.limits.talkPerDay - b.talkDay, doctrineDay: this.limits.doctrinePerDay - b.doctrineDay, briefingDay: this.limits.briefingPerDay - b.briefingDay };
  }
}
